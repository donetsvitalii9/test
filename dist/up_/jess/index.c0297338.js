...
})({"kREkd":[function(require,module,exports) {
"use strict";
var HMR_HOST = null;
var HMR_PORT = null;
var HMR_SECURE = false;
var HMR_ENV_HASH = "d6ea1d42532a7575";
module.bundle.HMR_BUNDLE_ID = "feb495fbc0297338";
...
},{}],"6UA96":[function(require,module,exports) {
var _cursor = require("./cursor");
var _utils = require("./utils");
var _gsap = require("gsap");
var _flip = require("gsap/Flip");
_gsap.gsap.registerPlugin(_flip.Flip);
// intro section
...
const openContent = (image, position)=>{
    ...
    const otherImages = images.filter((el)=>el != image
    ...
    _gsap.gsap.timeline({
        ...
        // Get state
        const state = _flip.Flip.getState(image);
        // Change place
        ...
        _gsap.gsap.set(contentBackCtrls[position], {
            xPercent: 20,
            opacity: 0
        });
        // hide text spans
        _gsap.gsap.set(contentTexts[position], {
            yPercent: 101
        });
    }, 'start').to([
        introTexts,
...
new _cursor.Cursor(document.querySelectorAll('.cursor'), 'a, .intro > .image, .content__back');
// Preload images and fonts and remove loader
Promise.all([
    _utils.preloadImages('.image__inner'),
    _utils.preloadFonts('jdl7wqk')
]).then(()=>document.body.classList.remove('loading')
);

},{"./cursor":"a0ePf","./utils":"WMAY0","gsap":"fPSuC","gsap/Flip":"iKp6p"}],"a0ePf":[function(require,module,exports) {
...
},{"./gsap-core.js":"05eeC","@parcel/transformer-js/src/esmodule-helpers.js":"gkKU3"}],"WMAY0":[function(require,module,exports) {
var parcelHelpers = require("@parcel/transformer-js/src/esmodule-helpers.js");
parcelHelpers.defineInteropFlag(exports);
...
},{}],"iKp6p":[function(require,module,exports) {
var parcelHelpers = require("@parcel/transformer-js/src/esmodule-helpers.js");
parcelHelpers.defineInteropFlag(exports);
...
_callbacks = _listToArray("onStart,onUpdate,onComplete,onReverseComplete,onInterrupt"), _removeProps = _listToArray("transform,transformOrigin,width,height,position,top,left,opacity,zIndex,maxWidth,maxHeight,minWidth,minHeight"), _getEl = function _getEl(target) {
    ...
    return cache.gMatrix = _matrixJs.getGlobalMatrix(el, true, false, true);
}, _getDOMDepth = function _getDOMDepth1(el, invert, level) {
    ...
    return parent ? l + _getDOMDepth1(parent, invert, level + 1) : l;
...
_makeAbsolute = function _makeAbsolute(elState, fallbackNode, ignoreBatch) {
    ...
    if (uncache) result = new ElementState(element);
        ...
        if (elState.simple) {
            ...
            result.matrix = new _matrixJs.Matrix2D(1, 0, 0, 1, bounds.left + _matrixJs._getDocScrollLeft(), bounds.top + _matrixJs._getDocScrollTop());
        } else result.matrix = _matrixJs.getGlobalMatrix(element, false, false, true);
    }
    ...
    if (simple || !parent) {
        ...
        matrix = _matrixJs.getGlobalMatrix(fitChild || element, false, false, true);
    ...
    var _vars = vars, clearProps = _vars.clearProps, onEnter = _vars.onEnter, onLeave = _vars.onLeave, absolute = _vars.absolute, absoluteOnLeave = _vars.absoluteOnLeave, custom = _vars.custom, delay = _vars.delay, paused = _vars.paused, repeat = _vars.repeat, repeatDelay = _vars.repeatDelay, yoyo = _vars.yoyo, toggleClass = _vars.toggleClass, nested = _vars.nested, _zIndex = _vars.zIndex, scale = _vars.scale, fade = _vars.fade, stagger = _vars.stagger, spin = _vars.spin, prune = _vars.prune, props = ("props" in vars ? vars : fromState).props, tweenVars = _copy(vars, _reserved), animation = gsap.timeline({
        ...
        return spinNum;
    }, interrupted = fromState.interrupted || toState.interrupted, addFunc = animation[relative !== 1 ? "to" : "from"], v, p1, endTime, i1, el1, comp1, state, targets, finalStates, fromNode, toNode, run, a, b; //relative || (toState = (new FlipState(toState.targets, {props: props})).fit(toState, scale));
    for(p1 in toState.idLookup){
        toNode = !toState.alt[p1] ? toState.idLookup[p1] : _getChangingElState(toState, fromState, p1);
        el1 = toNode.element;
        fromNode = fromState.idLookup[p1];
        fromState.alt[p1] && el1 === fromNode.element && (fromState.alt[p1].isVisible || !toNode.isVisible) && (fromNode = fromState.alt[p1]);
        if (fromNode) {
            comp1 = {
                t: el1,
                b: fromNode,
                a: toNode,
                sd: fromNode.element === el1 ? 0 : toNode.isVisible ? 1 : -1
            };
            comps.push(comp1);
            if (comp1.sd) {
                if (comp1.sd < 0) {
                ...
                interrupted && _recordInlineStyles(comp1.b, props ? _memoizedRemoveProps[props] : _removeProps);
                fade && comps.push(comp1.swap = {
                    t: fromNode.element,
                    b: comp1.b,
                    a: comp1.a,
                    sd: -comp1.sd,
            ...
            el1._flip = fromNode.element._flip = _batch ? _batch.timeline : animation;
            ...
            el1._flip = _batch ? _batch.timeline : animation;
        }
    }
    ...
    run = function run() {
        ...
        for(i1 = 0; i1 < comps.length; i1++){
            comp1 = comps[i1];
            a = comp1.a;
            b = comp1.b;
            if (prune && !a.isDifferent(b) && !comp1.entering) // only flip if things changed! Don't omit it from comps initially because that'd prevent the element from being positioned absolutely (if necessary)
            comps.splice(i1--, 1);
            else {
                el1 = comp1.t;
                nested && !(comp1.sd < 0) && i1 && (a.matrix = _matrixJs.getGlobalMatrix(el1, false, false, true)); // moving a parent affects the position of children
                if (comp1.sd || b.isVisible && a.isVisible) {
                    if (comp1.sd < 0) {
                        // swapping OUT (swap direction of -1 is out)
                        state = new ElementState(el1, props, fromState.simple);
                        _fit(state, a, scale, 0, 0, state);
                        state.matrix = _matrixJs.getGlobalMatrix(el1, false, false, true);
                        state.css = comp1.b.css;
                        comp1.a = a = state;
                        fade && (el1.style.opacity = interrupted ? b.opacity : a.opacity);
                        stagger && swapOutTargets.push(el1);
                    } else if (comp1.sd > 0 && fade) // swapping IN (swap direction of 1 is in)
                    el1.style.opacity = interrupted ? a.opacity - b.opacity : "0";
                ...
                if (!scale) {
                    el1.style.maxWidth = Math.max(a.width, b.width) + "px";
                    el1.style.maxHeight = Math.max(a.height, b.height) + "px";
                    el1.style.minWidth = Math.min(a.width, b.width) + "px";
                    el1.style.minHeight = Math.min(a.height, b.height) + "px";
                }
                nested && toggleClass && el1.classList.add(toggleClass);
            }
            finalStates.push(a);
        }
        var classTargets;
        ...
        if (custom && targets.length) {
            ...
            for(p1 in custom){
                v = _copy(custom[p1], _fitReserved);
                v[p1] = tweenVars[p1];
                !("duration" in v) && "duration" in tweenVars && (v.duration = tweenVars.duration);
                v.stagger = tweenVars.stagger;
                addFunc.call(animation, targets, v, 0);
                delete remainingProps[p1];
            }
        }
...
}, _interrupt = function _interrupt1(tl) {
    tl.vars.onInterrupt && tl.vars.onInterrupt.apply(tl, tl.vars.onInterruptParams || []);
    tl.getChildren(true, false, true).forEach(_interrupt1);
...
var FlipState = /*#__PURE__*/ function() {
    function FlipState1(targets, vars, targetsAreElementStates) {
    ...
    var _proto = FlipState1.prototype;
    ...
    _proto.fit = function fit(state, scale, nested) {
        ...
        for(; i < elStatesInOrder.length; i++){
            ...
            nested && (fromNode.matrix = _matrixJs.getGlobalMatrix(fromNode.element, false, false, true)); // moving a parent affects the position of children
            toNode = toElStates[fromNode.id];
            toNode && _fit(fromNode, toNode, scale, true, 0, fromNode);
            fromNode.matrix = _matrixJs.getGlobalMatrix(fromNode.element, false, false, true);
        }
        return this;
    };
    ...
    _proto.compare = function compare(state) {
        ...
        }, s11, s21, p, el2, s1Alt, s2Alt, c1, c2;
        for(p in l1){
            s1Alt = a1[p];
            s2Alt = a2[p];
            s11 = !s1Alt ? l1[p] : _getChangingElState(state, this, p);
            el2 = s11.element;
            s21 = l2[p];
            if (s2Alt) {
                c2 = s21.isVisible || !s2Alt.isVisible && el2 === s21.element ? s21 : s2Alt;
                c1 = s1Alt && !s11.isVisible && !s1Alt.isVisible && c2.element === s1Alt.element ? s1Alt : s11; //c1.element !== c2.element && c1.element === s2.element && (c2 = s2);
                ...
                placeIfDoesNotExist(c1.element !== s21.element && s1Alt ? s1Alt : c1, s21, s21.element);
                placeIfDoesNotExist(s1Alt && s1Alt.element === s2Alt.element ? s1Alt : c1, s2Alt, s2Alt.element);
                s1Alt && placeIfDoesNotExist(s1Alt, s2Alt.element === s1Alt.element ? s2Alt : s21, s1Alt.element);
            } else {
                !s21 ? enter.push(el2) : !s21.isDifferent(s11) ? unchanged.push(el2) : place(s11, s21, el2);
                s1Alt && placeIfDoesNotExist(s1Alt, s21, s1Alt.element);
            }
        }
        for(p in l2)if (!l1[p]) {
...
var ElementState = /*#__PURE__*/ function() {
    function ElementState1(element, props, simple) {
        this.element = element;
        this.update(props, simple);
    }
    var _proto2 = ElementState1.prototype;
    _proto2.isDifferent = function isDifferent(state) {
        var b1 = this.bounds, b2 = state.bounds;
        return b1.top !== b2.top || b1.left !== b2.left || b1.width !== b2.width || b1.height !== b2.height || !this.matrix.equals(state.matrix) || this.opacity !== state.opacity || this.props && state.props && JSON.stringify(this.props) !== JSON.stringify(state.props);
    };
    _proto2.update = function update(props, simple) {
        var self = this, element = self.element, getProp = gsap.getProperty(element), cache = gsap.core.getCache(element), bounds = element.getBoundingClientRect(), bbox = element.getBBox && typeof element.getBBox === "function" && element.nodeName.toLowerCase() !== "svg" && element.getBBox(), m = simple ? new _matrixJs.Matrix2D(1, 0, 0, 1, bounds.left + _matrixJs._getDocScrollLeft(), bounds.top + _matrixJs._getDocScrollTop()) : _matrixJs.getGlobalMatrix(element, false, false, true);
        ...
        self.ctm = element.getCTM && element.nodeName.toLowerCase() === "svg" && _matrixJs._getCTM(element).inverse();
        self.simple = simple || _round(m.a) === 1 && !_round(m.b) && !_round(m.c) && _round(m.d) === 1; // allows us to speed through some other tasks if it's not scale/rotated
        self.uncache = 0;
    };
    return ElementState1;
}();
var FlipAction = /*#__PURE__*/ function() {
    function FlipAction1(vars, batch) {
        this.vars = vars;
        this.batch = batch;
        this.states = [];
        this.timeline = batch.timeline;
    }
    var _proto3 = FlipAction1.prototype;
...
var FlipBatch = /*#__PURE__*/ function() {
    ...
    var _proto4 = FlipBatch1.prototype;
...
var Flip = /*#__PURE__*/ function() {
    function Flip1() {}
    Flip1.getState = function getState(targets, vars) {
        var state = _parseState(targets, vars);
        _batchAction && _batchAction.states.push(state);
        vars && vars.batch && Flip1.batch(vars.batch).state.add(state);
        return state;
    };
    Flip1.from = function from(state, vars) {
        vars = vars || {};
    ...
    Flip1.to = function to(state, vars) {
        return _fromTo(state, _parseState(vars.targets || state.targets, {
            props: vars.props || state.props,
            simple: vars.simple,
            kill: !!vars.kill
    ...
    Flip1.fromTo = function fromTo(fromState, toState, vars) {
        return _fromTo(fromState, toState, vars);
    };
    Flip1.fit = function fit(fromEl, toEl, vars) {
    ...
    Flip1.makeAbsolute = function makeAbsolute(targetsOrStates, vars) {
        return (targetsOrStates instanceof FlipState ? targetsOrStates : new FlipState(targetsOrStates, vars)).makeAbsolute();
    };
    Flip1.batch = function batch(id) {
        id || (id = "default");
        return _batchLookup[id] || (_batchLookup[id] = new FlipBatch(id));
    };
    Flip1.killFlipsOf = function killFlipsOf(targets, complete) {
        (targets instanceof FlipState ? targets.targets : _toArray(targets)).forEach(function(t) {
            return t && _killFlip(t._flip, complete !== false ? 1 : 2);
        });
    };
    Flip1.isFlipping = function isFlipping(target) {
        var f = Flip1.getByTarget(target);
        return !!f && f.isActive();
    };
    Flip1.getByTarget = function getByTarget(target) {
        return (_getEl(target) || _emptyObj)._flip;
    };
    Flip1.getElementState = function getElementState(target, props) {
        return new ElementState(_getEl(target), props);
    };
    Flip1.convertCoordinates = function convertCoordinates(fromElement, toElement, point) {
        var m = _matrixJs.getGlobalMatrix(toElement, true, true).multiply(_matrixJs.getGlobalMatrix(fromElement));
        return point ? m.apply(point) : m;
    };
    Flip1.register = function register(core) {
        _body = typeof document !== "undefined" && document.body;
        if (_body) {
            gsap = core;
            _matrixJs._setDoc(_body);
...
Flip.version = "3.10.3"; // function whenImagesLoad(el, func) {
// 	let pending = [],
// 		onLoad = e => {
...
},{"@parcel/transformer-js/src/esmodule-helpers.js":"gkKU3"}]},["kREkd","6UA96"], "6UA96", "parcelRequiref8c0")

//# sourceMappingURL=index.c0297338.js.map
